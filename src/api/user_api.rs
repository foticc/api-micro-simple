...
use crate::service::user_service::{
    ChangePassword, CreateUser, SearchParams, UpdateUser, UserService,
};
...
#[get("/auth-code/{id}")]
pub async fn find_one_auth_code(
    state: Data<AppState>,
    path: Path<i32>,
...
pub async fn list(
    state: Data<AppState>,
    Json(page): Json<FilterParam<SearchParams>>,
) -> Result<impl Responder, UserError> {
...
#[put("/psd")]
pub async fn modify_psd(
    state: Data<AppState>,
    Json(pwd): Json<ChangePassword>,
) -> Result<impl Responder, UserError> {
