...
impl UserService {
    pub async fn create_user(state: Data<AppState>, user: CreateUser) -> Result<Model, UserError> {
        ...
        let user_role: Vec<crate::entity::sys_user_role::ActiveModel> = user
            .role_id
            .iter()
            .map(|&m| crate::entity::sys_user_role::ActiveModel {
                id: NotSet,
                role_id: Set(m),
                user_id: Set(x.id),
                updated_at: NotSet,
        ...
        SysUserRole::insert_many(user_role).exec(&txn).await?;
        txn.commit().await?;
        Ok(x)
    }

    pub async fn find_one_by_user_name(
        state: Data<AppState>,
        user_name: String,
    ) -> Result<UserName, DbErr> {
    ...
    pub async fn find_all(
        ...
        paginator
            .fetch_page(page.page_index - 1)
            .await
            .map(|list| PageResult::new(page.page_index, page.page_size, list, total))
    }
    ...
    pub async fn find_one(state: Data<AppState>, id: i32) -> Result<UserDto, DbErr> {
        ...
        let option = User::find_by_id(id).one(&state.conn).await?;
        if option.is_none() {
    ...
    pub async fn update(
        ...
        let sys_user_role: Vec<crate::entity::sys_user_role::ActiveModel> = update_user
            .user
            .role_id
            .iter()
            .map(|&m| crate::entity::sys_user_role::ActiveModel {
        ...
        if !sys_user_role.is_empty() {
            SysUserRole::insert_many(sys_user_role).exec(&txn).await?;
        }
    ...
    pub async fn change_pwd(state: Data<AppState>, pwd: ChangePassword) -> Result<(), UserError> {
        ...
        let option = User::find_by_id(user_id).one(&state.conn).await?;
        if option.is_none() {
